...
pub fn commit_changes(repo_path: &Path, todo: &TodoItem, _changes: &[String]) -> Result<String> {
    ...
    // Check if branch exists, and if so, add a timestamp suffix
    let branch_check = Command::new("git")
        .args(["rev-parse", "--verify", &base_branch_name])
        .current_dir(repo_path)
        .output()?;

    let branch_name = if branch_check.status.success() {
        ...
        let timestamp = SystemTime::now()
            ...
            .unwrap()
            .as_secs();
        format!("{}-{}", base_branch_name, timestamp)
    } else {
        base_branch_name
    };
